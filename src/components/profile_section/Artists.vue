<template>
    <ul class="nav nav-pills">
        ...
        <li class="nav-item">
            <router-link to="/profile/artists" class="nav-link fw-bold theme-btn  text-white">Artists</router-link>
        </li>
        <li class="nav-item">
    ...
    <div class="d-flex justify-content-end mt-2">
        <button v-show="searchFinished" type="button" class="btn theme-btn  text-white fw-bold" @click="get_artists">Load
            more</button>
...
async function get_artists() {
    ...
    let data = await ft.API(`/profile/artists/${offset.value}/${total.value}`);
    if (!data) return;
